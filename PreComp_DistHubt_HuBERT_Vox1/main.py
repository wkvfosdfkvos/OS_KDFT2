...
def run(process_id, args, experiment_args):
    ...
    #==========================================
    # model
    model.set_module_trainability('backend', False)
    ...
    if logger is not None:
        for key, v in model.copy_state_dict().items():
            logger.save_model(f'KD_fin_{key}', v)
    ...
    model.set_module_trainability('backend', True)
        ...
        if eer < best_eer:
            ...
            if logger is not None:
                for k, v in best_state.items():
                    logger.save_model(f'check_point_{k}_{epoch}', v)
        if logger is not None:
