...
def run(process_id, args, experiment_args):
    ...
    # criterion
    ssl_kd_criterion = loss.DistilHubertKDLoss(
        args['cos_lambda'], 
        args['target_layer_idx'], 
        args['student_hidden_layer_size'] 
    )
    clf_kd_criterion = torch.nn.MSELoss()
    ...
    # model
    model = experiment.Model(args, data_manager.vox1.NUM_TRAIN_SPK)
    model.set_kd_criterion(ssl_kd_criterion, clf_kd_criterion)
    ...
    model.set_module_trainability('backend_T', True)
    model.set_module_trainability('ft_loss', True)

    model.set_module_trainability('student', False)
    model.set_module_trainability('backend_S', False)
    model.set_module_trainability('ssl_kd_loss', False)
    model.set_module_trainability('clf_kd_loss', False)
    ...
    for epoch in range(1, args['epoch'] + 1):
        if epoch == 6:
            model.set_module_trainability('teacher', True)
            # optimizer
            optimizer = torch.optim.Adam(
                model.get_parameters(),
                lr=args['lr'], 
        ...
        experiment.train_ft(
            current_epoch=epoch,
            model=model,
            data_loader=data_manager.train_loader_ft,
            logger=logger
        )
        ...
        if logger is not None:
            logger.log_metric('eer/val_teacher', eer, epoch)
    ...
    if logger is not None:
        logger.log_metric('eer/eval_teacher', eer, epoch)
    ...
    # model
    model.set_module_trainability('teacher', False)
    model.set_module_trainability('backend_T', False)
    model.set_module_trainability('ft_loss', False)

    model.set_module_trainability('student', True)
    model.set_module_trainability('backend_S', True)
    model.set_module_trainability('ssl_kd_loss', True)
    model.set_module_trainability('clf_kd_loss', True)
    model.test_teacher = False
    ...
    best_eer = 100
    for epoch in range(1, args['epoch_KD'] + 1):
        if int(args['epoch_KD'] * 0.9) == epoch:
            data_manager.change_KD_train_set()
        ...
        experiment.train_kd(
            current_epoch=epoch,
            model=model,
            data_loader=data_manager.train_loader_kd,
            logger=logger
        )
        ...
        if eer < best_eer:
            ...
            if logger is not None:
                for k, v in best_state.items():
                    logger.save_model(f'KD_checkpoint[{epoch}]_{k}', v)
        if logger is not None:
